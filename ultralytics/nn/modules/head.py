...
from .conv import Conv, PConv, PConv1
...
__all__ = 'Detect', 'Segment', 'Pose', 'Classify', 'RTDETRDecoder','PDetect','v10Detect'
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], self.nc)  # channels
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), PConv(c2, 3, 4, 'split_cat'), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch)
        self.cv3 = nn.ModuleList(
            nn.Sequential(Conv(x, c3, 3), PConv(c3, 3, 4, 'split_cat'), nn.Conv2d(c3, self.nc, 1)) for x in ch)
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
